...
**Headless console launcher: `openDurableConsole` / `mintDurableConsoleSession` / `durableConsoleSessionUrl`, exported from `@dudousxd/nestjs-durable-dashboard/client`.**
...
await openDurableConsole({ headers: () => ({ Authorization: `Bearer ${token()}` }) });
```
