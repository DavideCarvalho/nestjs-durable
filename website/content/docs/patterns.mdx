...
export class TripBookingWorkflow {
  ...
  async run(ctx: WorkflowCtx, trip: TripRequest) {
    const flight = await ctx.localStep(
      'reserve-flight',
      () => this.flights.reserve(trip.flightId),
      { compensate: () => this.flights.release(trip.flightId) },
    );
    const hotel = await ctx.localStep(
      'reserve-hotel',
      () => this.hotels.reserve(trip.hotelId),
      { compensate: () => this.hotels.release(trip.hotelId) },
    );
    // If this throws, the engine undoes the hotel then the flight reservation before failing.
    const deposit = await ctx.localStep('charge-deposit', () =>
      this.billing.chargeDeposit(trip.customerId, trip.depositCents),
    );
...
Each `ctx.localStep` that has a real-world side effect registers its own `compensate` undo; if a
later step fails (exhausts retries, or throws `FatalError`), the engine runs every registered
compensation in reverse order before failing the run — here, release the hotel, then the flight.
Sagas attach only to `ctx.localStep` (in-process), not the dispatched `ctx.step`. This is the short
version — for retryable undos, visibility, and compensating cancellation, see
