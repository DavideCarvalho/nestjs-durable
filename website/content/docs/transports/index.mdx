---
title: Overview
description: How steps travel to workers. From an in-process event-emitter for zero-infra single-process handlers, to BullMQ/Redis, SQS, and a broker-less SQL transport for cross-process and cross-language steps.
---

A **transport** is how `ctx.step` reaches its `@Step` handler and how the result comes back. It's a
...
| --- | --- | --- |
| `EventEmitterTransport` | No (in-process) | The zero-infra default: `@Step` handlers run in the **same process**, decoupled from the workflow. No broker. |
...
Backed by `@nestjs/event-emitter`. Handlers are `@Step` provider methods; the engine
...
The engine dispatches each step on the **first** transport in the pool and **fails over** to
...
const result = await ctx.step(this.pipeline.heavyStep, input, { transport: 'sqs' });
```
