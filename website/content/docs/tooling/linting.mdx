...
`@Step` handler that runs once and checkpoints its result) is fine, because those aren't
replayed.
...
guidance to switch to `ctx.now()` / `ctx.sideEffect(...)`. Because the scope is the *file*
