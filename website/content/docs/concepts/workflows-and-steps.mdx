...
description: Declaring workflows with @Workflow, local steps with ctx.step, typed remote steps with remoteStep + ctx.remote and @DurableStep handlers, retries, fan-out and fatal errors.
---
...
Declare a typed remote step with `remoteStep`, then call it with `ctx.remote` — the engine
...
- A **step** (`ctx.step` / `ctx.remote`) is a **durable checkpoint**. The engine records its result, so
