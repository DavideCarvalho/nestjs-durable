...
[`CONTROL_FLOW_SIGNAL`](/docs/api-reference#control-flow-signals), and the exported
...
[Troubleshooting: a caught suspend runs compensation and corrupts the run](/docs/troubleshooting#a-caught-suspend-runs-compensation-and-corrupts-the-run).
...
which re-exports it) — see [API reference](/docs/api-reference#control-flow-signals).
...
- **It never waits behind a queue.** A compensation dispatch skips flow-control on purpose: the saga is
  often unwinding *because* the system is under pressure, and the very `queue` that throttled (or failed)
  the forward work must not be able to starve its own undo. Compensations always dispatch directly, by
  name.
- **It is always a durable dispatch.** An ordinary step's `timeoutMs` switches it onto the in-memory
  liveness path (held await + heartbeats — traded against durable suspension). An unwind must never make
  that trade: a compensation is dispatched durably and waited on exactly like a step with no `timeoutMs`,
  so a crash mid-undo resumes instead of losing the wait.

Either way the unwind is checkpointed and crash-safe — but plan undo-handler capacity accordingly if a
saga can fire under load.
