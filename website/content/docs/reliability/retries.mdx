...
- `remoteRedispatchMs` — an **opt-in** engine option — makes this self-healing: a dispatched remote
  ...
  heartbeat, which **renews** the lease — must land before it lapses; a lapsed lease re-dispatches the
  same `stepId`, bounded by `remoteRedispatchMax` (default 10) before failing the step as
  `remote_step_lost` (which reaches the workflow's own error path). Off by default, because
