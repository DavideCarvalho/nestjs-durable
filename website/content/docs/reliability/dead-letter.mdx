...
The walkthrough below follows one poison-pill run the whole way: it crashes, crash-recovery keeps
resuming it until it hits `maxRecoveryAttempts`, the engine moves it to the terminal `dead` status —
and then a `@DeadLetter()` handler picks it up as a durable run of its own to page and file a ticket.
Note the two lanes: the poisoned run **ends** on its lane — dead, in red — while the DLQ handler is a
separate run living on the lane below.

<CodeFlow scene="dead-letter" />

And as a live system — inject a poison pill and watch it crash-loop, get dead-lettered, and hand off
to the DLQ handler while the healthy traffic never stops:
...
export class PipelineWorkflow {
  ...
  async run(ctx: WorkflowCtx, input: PipelineInput) {
    ...
    const transformed = await ctx.step('pipeline.transform', extracted);
    return ctx.step('pipeline.load', transformed);
  }
  ...
  async onDead(ctx: WorkflowCtx, dl: DeadLetter<PipelineInput>) {
    await ctx.step(this.alerts.page, {
      event: 'durable-dead-letter',
      runId: dl.deadRunId,
      workflow: dl.workflow,
      error: dl.error?.message,
    });

    const ticket = await ctx.step(this.tickets.create, {
