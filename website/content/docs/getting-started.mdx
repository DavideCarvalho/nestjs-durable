...
Inject `WorkflowService` and start the workflow. It runs the local + remote steps, then suspends
...
async checkout(order: Order) {
  const run = await this.workflows.start('checkout', order); // → { status: 'suspended', ... }
  return run.runId;
}
