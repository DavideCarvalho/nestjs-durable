...
- `GET runs` takes `limit` / `offset`, and the SPA asks for 100 rows at a time — **show more** raises
  the ceiling for an operator scrolling further back. Every store adapter implements both.
