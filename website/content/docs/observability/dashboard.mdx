...
That is also why the origin facet is counted **in the browser**, over the list the console already
holds, while tenant is pushed to the store. A store-side facet would have reported zero unattributed
runs the moment an operator picked a library — which reads as "those runs do not exist". The
server-side `origin` param is still wired and still sent with bulk actions, so **retry all** /
**cancel all** stay scoped to the list they were launched from; under the `unknown` facet — the one
filter the server cannot be told about — those buttons are disabled rather than quietly widening to
every origin.
