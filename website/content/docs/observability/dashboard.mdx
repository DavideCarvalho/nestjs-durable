...
- **Opt-in self-heal — `remoteRedispatchMs`** (any transport, any consumer). Set this engine option and
  ...
  heartbeat, which renews the lease — must land before it lapses, and a lapsed lease re-dispatches the
  step automatically, bounded by `remoteRedispatchMax` (default 10) so a step that never settles fails
