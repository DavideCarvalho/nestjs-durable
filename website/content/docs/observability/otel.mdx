...
The [Telescope watcher](/docs/observability/telescope) trace-groups a run the same way (its own
root span per run, ended on `run.suspended`) and hits the identical boundary — a run that suspends
splits into more than one `traceId` on Telescope's Traces page too. What Telescope adds is a second,
span-independent way to see the whole run: every entry it records also carries a `run:<runId>` tag,
so filtering the Entries view by that tag reconstructs the full lifecycle — across any number of
suspends — regardless of how the OTel trace was split. The [dashboard](/docs/observability/dashboard)'s
run timeline is unaffected either way: it's built from the state store's checkpoints, not from spans.
</Callout>
