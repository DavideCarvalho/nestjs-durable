...
run.tags.includes('version:undeclared')  // true — so a pin check can say so
```

Read that tag before treating `workflowVersion` as something a pin was checked against. It travels on
