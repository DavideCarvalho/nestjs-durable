...
itself checkpointed, and only then `await hook.wait()`. Pass `{ timeoutMs }` to bound the wait —
past the deadline it throws `SignalTimeoutError`, so an abandoned callback becomes a branch you
handle instead of a run parked forever. `wait()` parks the run on the same logical
