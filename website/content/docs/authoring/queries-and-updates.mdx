...
dispatched step, parked on a signal — you often need to **read** what it has done so far, and sometimes
...
The walkthrough below traces a video-encode run publishing progress with `ctx.setEvent` as it works,
while an outside controller reads the latest snapshot with `engine.getEvent`. Watch the external
*read* land against a run that is otherwise busy — and leave it completely undisturbed.
...
export class VideoEncodeWorkflow {
  ...
  async run(ctx: WorkflowCtx, job: EncodeJob) {
    ...
    for (let i = 0; i < segments.length; i++) {
      await ctx.step(this.encoder.encodeSegment, { jobId: job.id, segment: segments[i] });
...
export class ExpenseApprovalWorkflow {
  ...
  async run(ctx: WorkflowCtx, expense: Expense) {
    ...
    await ctx.step(this.ledger.reimburse, { expense, approver: decision.approver });
...
The `{ timeoutMs }` is what keeps a parked run from waiting forever. If nobody decides in time, the
wait ends on **its own terms** — `onUpdate` throws `SignalTimeoutError`, the workflow catches it, and
the run reaches a clean terminal state instead of sitting suspended indefinitely:
...
Here is the whole happy path end to end — the parked run on the left, the controller's
`engine.update` command on the right — validate, deliver, resume, complete:
