...
The walkthrough below traces a video-encode run that does exactly this: it publishes progress with
`ctx.setEvent` as it works, and pauses at a `ctx.onUpdate` point where an outside caller can bump its
priority mid-run. Watch the two highlighted interaction beats — an external *read* of progress and an
external *update* of priority — happen against a run that is otherwise busy.
