...
- **Crash recovery is a lease, not a boot-time sweep.** While a run executes, its worker renews a
  ...
  boot. `leaseMs` defaults to 30 000 (`packages/core/src/engine.ts:226`,
  `this.leaseMs = deps.leaseMs ?? 30_000` at `packages/core/src/engine.ts:400`) — set it above your
...
`maxRecoveryAttempts` is unlimited by default (`packages/nestjs/src/durable.module.ts:241`) — recovery
...
`WorkflowRegistrar.onApplicationShutdown` calls `engine.drain(this.options.shutdownTimeoutMs)`
(`packages/nestjs/src/workflow.registrar.ts:67`) — `shutdownTimeoutMs` defaults to 10 000
(`packages/nestjs/src/durable.module.ts:271`). `drain` sets an internal flag that turns recovery and
the pending-run poll into no-ops (`packages/core/src/engine.ts:1093`), so no *new* run starts, then
...
await a remote step while it drains (`packages/nestjs/src/workflow.registrar.ts:61-64`). Only after
...
a pitfall of a stale local worker in [Tenancy & topologies](/docs/concepts/tenancy#pitfalls). The lint
...
process. See [Tenancy & topologies](/docs/concepts/tenancy) for the full operator/tenant model and its
pitfalls.
...
- **OpenTelemetry** (`@dudousxd/nestjs-durable-otel`) turns runs into traces — one root span per run,
  ...
  scheduled exporter (`packages/otel/src/durable-metrics.ts`).
...
`packages/store-mikro-orm/src/schema.ts:174-186`) so concurrent pods don't race the same DDL.
...
`autoSchema` defaults to `true` (`packages/nestjs/src/durable.module.ts:219`). The same additive,
