...
This page is a flat lookup table generated from source (`packages/core/src/interfaces.ts`,
`packages/core/src/engine.ts`, `packages/core/src/errors.ts`, `packages/core/src/scheduler.ts`,
`packages/nestjs/src/workflow.service.ts`, `packages/nestjs/src/durable.module.ts`, and the
`@dudousxd/nestjs-durable` decorators). It is deliberately terse — for the *why* behind any entry,
follow its linked concept/authoring/reliability page.
...
| `ChildCallOptions` | `{ childId?, priority? }` | Options for `ctx.child`/`ctx.startChild` (a bare string is shorthand for `{ childId }`). `priority` only affects a remote child. |
...
Optional, TYPES-ONLY contract (`packages/core/src/workflow-handler.ts`, exported from
...
Optional base class (`packages/core/src/durable-workflow.ts`) — `@Workflow` classes that extend it
...
| Member | Signature | Notes |
| --- | --- | --- |
| `start` | `start(workflow: WorkflowRef, input: unknown, runId: string, opts?: StartOptions): Promise<RunResult>` | Create + enqueue a run. Idempotent by `runId` — a redelivered `runId` returns the existing run's state. |
...
| `sweepTimeouts` | `sweepTimeouts(now?: number): Promise<void>` | Cancel in-flight runs that outlived their `@Workflow({ executionTimeout })`. |
