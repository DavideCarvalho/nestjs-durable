---
title: Python steps
description: Run workflow steps in Python. A TypeScript workflow's ctx.remote dispatches to a Python worker over BullMQ/Redis, and the result flows back — one workflow, steps split across languages.
---
...
- **The engine owns correlation.** Every result carries the `stepId` it answers; the engine matches
  it to the suspended `ctx.remote` and resumes the run. The worker never needs the run's state.
