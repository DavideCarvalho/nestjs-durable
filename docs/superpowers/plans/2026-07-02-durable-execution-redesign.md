...
- Modify: `packages/worker/src/workflow-context.ts` (`resolveCallGroup` :186-190 — the worker-side decision emission that currently reads `step.group`; **this is a compile break after Task 2** since `RemoteStepDef.group` is gone)
- Test: `packages/worker/src/redis-runner.spec.ts`, `workflow-worker.spec.ts`, `step-worker.spec.ts`, `workflow-context.spec.ts`

**Cross-SDK decision protocol (transitional wire contract — read this):**
The `group` field is overloaded across two message stages during the deprecation window:
- On a **dispatched task** (`RemoteTask`/`WorkflowTask`), it carries the FINAL queue token `tenantGroup(sanitizeQueueToken(name), partition)` (set by the core engine, Task 2).
- On a **`call` decision** emitted by a replaying worker (this package's `WorkflowContext`, and the Python `WorkflowContext` in Task 8), it carries the **partition only** — the engine composes the final token with `tenantGroup(sanitizeQueueToken(cmd.name), cmd.group)` (see `engine.ts` `cmd.group` sites updated in Task 2). So the worker's decision must emit the step's `partition`, NOT a group and NOT the full token (else the engine double-wraps).
Therefore `resolveCallGroup` collapses: the old "explicit-group-wins / else workflow-group / else dot-prefix" logic is DELETED; it returns `step.partition ?? this.workflowPartition` (the `workflowGroup` field becomes `workflowPartition`, optional). The Python analog is verified/aligned in Task 7.
...
**Interfaces:**
- Produces: `Worker(partition=None, …)`. **Semantics:** Python had TWO axes — `group` (the base queue, e.g. `processing`) and `tenant` (the isolation suffix `@davi-local`). The redesign ELIMINATES the base (it becomes the handler name) and RENAMES the suffix. So: the **isolation suffix `tenant=` becomes `partition=`** — keep `tenant=` as a `@deprecated` keyword alias that maps to `partition` for ROUTING (`warnings.warn`); `Worker.tenant` (start-run stamping) still reflects it. The old base **`group=` is deprecated and IGNORED for routing** (the base is now the handler name) — accept it with a `warnings.warn`, do not map it to partition. `run_redis_worker` starts one `BullWorker` per name in `self._handlers.keys() ∪ self._workflows.keys()`, each on `f"{prefix}-tasks-{tenant_group(sanitize_queue_token(name), partition)}"`. `sanitize_queue_token(name) = name.replace(':', '-')` — must byte-match JS.
...
- [ ] **Step 3: Implement.** Add `sanitize_queue_token` to `redis_runner.py`. `Worker.__init__`: add `partition=None`; the isolation suffix comes from `partition`. Accept deprecated `tenant=` kw → if passed and `partition` is not, `warnings.warn` and set `partition = tenant` (so `Worker(tenant='davi-local')` still routes to `@davi-local`); keep stamping `Worker.tenant` from whichever is set. Accept deprecated positional/kw `group=` → `warnings.warn` and IGNORE for routing (base is now the handler name). In `run_redis_worker` (`:382-468`): replace the single-queue build with a loop over the union of `runtime`'s registered handler + workflow names, each `tasks_name = f"{effective_prefix}-tasks-{_tenant_group(sanitize_queue_token(name), partition)}"`, one `BullWorker` per name sharing `process`. Keep `_names`'s format string identical; results queue stays single. Preserve the `_effective_prefix`/`_tenant_group` byte-contract.
