...
- 4abdd5b: Assert this adapter's column names against the cross-adapter contract
...
- 9ae1bf3: Enforce the `namespace` isolation boundary in the TypeORM adapter.
...
- 9d9bae1: Fix MySQL TEXT (64KB) truncation: store run/checkpoint JSON blobs (`input`/`output`/`error`/`events`)
