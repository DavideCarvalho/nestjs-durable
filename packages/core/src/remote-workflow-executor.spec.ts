...
function run(): WorkflowRun {
  return { id: 'r1', workflow: 'greet', workflowVersion: '1', input: 'davi', status: 'running', createdAt: new Date(), updatedAt: new Date() };
}
...
describe('RemoteWorkflowExecutor', () => {
  it('dispatches a workflow task and resolves the matching decision (correlated by taskId)', async () => {
    ...
    const transport: Transport = {
      ...
      onResult: () => {},
      onHeartbeat: () => {},
      dispatchWorkflowTask: async (t) => { dispatched = t; },
      onDecision: (h) => { decide = h; },
    };
    ...
    await decide!({ taskId: dispatched!.taskId, runId: 'r1', status: 'completed', commands: [], output: { msg: 'hi' } });
