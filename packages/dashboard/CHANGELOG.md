...
- f7716c2: Value pickers are exact, searchable and paged — not a top-100 sample
...
- 5f17363: `useOpenDurableConsole` stops sticking on `isPending` when the caller navigates
...
- 636693e: Three fixes to the console's value pickers, from watching one on real data
...
- 0f67ad8: Require the filter release that makes the console's own URLs parse
...
- 382f34d: The console wires its filter library privately — `filterModule` is gone
...
- 1171860: The console's tag, tenant and attribute filters are pickers now, and take several values
...
- bcd31af: Fix the console's run list rendering with overlapping rows and gaps after a filter change
...
- 39955d4: The console's run list scrolls infinitely instead of asking for the next page
...
- 0203613: Say which tenant and which library produced a run — and filter by both.
...
- 65a759f: Adopt the shared Aviary console tokens and put the console's primitives on shadcn + Base UI.
  ...
  - Fix &amp; replay edits the run input in a proper dialog with a resizable JSON editor. It used to be
    ...
    error only _after_ the edit had already been discarded. The dialog keeps the draft and shows the
    parse error inline.
...
- f18b909: Durable console accent moves from emerald `#34d399` to lime `#a3e635`.

  Emerald _is_ `--good`. Using it as the accent gave one hue two jobs — "this step succeeded" and "this is interactive".

  Durable picked last and had the least room left on the wheel, because its own local hues consume space too: bad 0°, no-worker 25°, warn 45°, good 160°, media cyan 187°, live 217°, sleeping 239°, agent violet 258°, telescope magenta 300°, dead 347°. Only two arcs survived. Lime sits mid-arc at 82° — 37° from `--warn`, 78° from `--good`.

  Pink `#f472b6` separates further and was rejected on purpose: a pink primary action sits on the same row as a red FAILED badge and borrows an alarm it does not mean.

## 0.39.2

### Patch Changes

- 0989247: Fix the console launcher hanging on a spinner after the user presses Back.
