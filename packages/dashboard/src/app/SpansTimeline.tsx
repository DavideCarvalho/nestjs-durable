...
export function RunSpans({
  ...
  const renderRow = ({ step, left, width, ms, subRows }: (typeof rows)[number]) => {
  ...
  return (
    <>
      {nodes.map((node) => {
        ...
        // A parallel fan: lay its member steps out as siblings under one labelled header, each with
        // its own status/duration bar (same row styling), to read as "these N ran in parallel".
        const memberRows = node.steps
          .map((s) => rowBySeq.get(s.seq))
          .filter((r): r is (typeof rows)[number] => r !== undefined);
        const fanFailed = node.steps.some((s) => s.status === 'failed');
        return (
          <div
            key={`fan:${node.group}`}
            className="rounded-md bg-zinc-900/30 ring-1 ring-[var(--line-soft)]"
          >
            <div className="flex items-center gap-2 px-2 py-1">
              ...
              <span className="mono text-[10px] uppercase tracking-[0.15em] text-zinc-500">
                parallel
              </span>
            ...
            <div className="ml-[10px] border-l border-[var(--line-soft)] pl-1">
              {memberRows.map((row) => renderRow(row))}
            </div>
          </div>
        );
      })}
