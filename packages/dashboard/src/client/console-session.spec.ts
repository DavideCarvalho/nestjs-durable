...
describe('durableConsoleSessionUrl', () => {
  it('derives the mint path from the default mount', () => {
    expect(durableConsoleSessionUrl()).toBe('/durable/session');
    expect(durableConsoleUrl()).toBe('/durable');
  });
  ...
  it('follows a custom basePath', () => {
    ...
    expect(durableConsoleSessionUrl('/ops/workflows')).toBe('/ops/workflows/session');
    expect(durableConsoleUrl('/ops/workflows')).toBe('/ops/workflows');
  });

  it('tolerates a missing leading slash and a trailing one', () => {
    expect(durableConsoleSessionUrl('ops/')).toBe('/ops/session');
    expect(durableConsoleUrl('ops/')).toBe('/ops');
  });
});

describe('mintDurableConsoleSession', () => {
  it('posts with credentials so the Set-Cookie sticks', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await mintDurableConsoleSession({ fetch: fetchMock });

    const [url, init] = firstCall(fetchMock);
    expect(url).toBe('/durable/session');
  ...
  it('sends host headers, resolving a function at call time', async () => {
    ...
    await mintDurableConsoleSession({
      fetch: fetchMock,
  ...
  it('throws with the status when the console refuses', async () => {
    ...
    await expect(mintDurableConsoleSession({ fetch: fetchMock })).rejects.toBeInstanceOf(
      ConsoleSessionError,
    );
    await expect(mintDurableConsoleSession({ fetch: fetchMock })).rejects.toMatchObject({
      status: 401,
      url: '/durable/session',
    });
  });
  ...
  it('reports a browser opaque redirect as an interception, not a success', async () => {
    ...
    await expect(mintDurableConsoleSession({ fetch: fetchMock })).rejects.toThrow(
...
describe('openDurableConsole', () => {
  ...
  it('navigates to the console after a successful mint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await openDurableConsole({ fetch: fetchMock, navigate });
  ...
  it('navigates to the custom mount', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response());
    await openDurableConsole({ fetch: fetchMock, navigate, basePath: '/ops/workflows' });

    expect(navigate).toHaveBeenCalledWith('/ops/workflows');
  });

  it('does NOT navigate when the mint is refused', async () => {
    const fetchMock = vi.fn().mockResolvedValue(response({ status: 403 }));

    await expect(openDurableConsole({ fetch: fetchMock, navigate })).rejects.toBeInstanceOf(
      ConsoleSessionError,
    );
  ...
  it('does NOT navigate when the response was an intercepted redirect', async () => {
    ...
    await expect(openDurableConsole({ fetch: fetchMock, navigate })).rejects.toThrow(
      /answered with a redirect/,
    );
    expect(navigate).not.toHaveBeenCalled();
  });
