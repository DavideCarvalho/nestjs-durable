...
const DEFAULT_BASE_PATH = '/durable';
 ...
 *
 * Derived from `basePath` rather than hardcoded by the caller: this package owns the route (the
 * auth controller is mounted relative to `basePath` by `RouterModule`), so a host that hardcodes it
 ...
 */
export function durableConsoleSessionUrl(basePath: string = DEFAULT_BASE_PATH): string {
  return `${normalizeBasePath(basePath)}/session`;
}

/** Where the console SPA itself is served. */
export function durableConsoleUrl(basePath: string = DEFAULT_BASE_PATH): string {
  return normalizeBasePath(basePath) || '/';
}

export interface OpenConsoleOptions {
  /** Where the console is mounted. MUST match `DurableDashboardModule.forRoot({ basePath })`. */
  basePath?: string;
  /**
 ...
 */
export async function mintDurableConsoleSession(options: OpenConsoleOptions = {}): Promise<void> {
  const url = durableConsoleSessionUrl(options.basePath);
 ...
 */
export async function openDurableConsole(options: OpenConsoleOptions = {}): Promise<void> {
  await mintDurableConsoleSession(options);
  const target = durableConsoleUrl(options.basePath);
